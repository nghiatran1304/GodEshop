...
public interface ProductDAO extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT new com.godEShop.Dto.ProductShopDto"
	    + "(p.id, c.id, p.name, p.price, p.createDate, c.name, MIN(pp.id), CAST(AVG(pe.evaluation) AS int), pd.discount, p.detail, MAX(pd.endDate)) "
	    + "FROM Product p " 
	    ...
	    + "GROUP BY p.id, c.id, p.name, p.price, p.createDate, c.name, pd.discount, p.detail")
